#!/usr/bin/env python3
"""Load a Wan UniLoRA checkpoint and reconstruct a video clip from a text prompt."""
...
from pipeline_wan_scaling_encode import WanPipeline
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--weights",
        type=Path,
        help="Path to UniLoRA .safetensors file",
    )
    parser.add_argument(
        "--disable_lora",
        action="store_true",
        help="Disable LoRA adaptation and use base model only",
    )
    ...
    parser.add_argument("--name", type=str, default=None, help="Name of the video")
    parser.add_argument(
        "--scaling",
        action="store_true",
        default=False,
        help="Enable importance-scaling candidate selection during denoising",
    )
...
def load_reference_latent(
    ...
    """Load frames, convert to video tensor, and encode to latents.
    ...
    Args:
        ...
        vae: VAE model to encode the video
    
    Returns:
        Latent tensor with shape [1, C, T, H, W] in latent space
    """
    if not frames_dir.exists():
    ...
    video = torch.stack(pixels, dim=0).permute(1, 0, 2, 3).unsqueeze(0).to(device)
    
    # Encode video to latents
    with torch.no_grad():
        # VAE expects [B, C, T, H, W] format in [-1, 1] range
        video = video.to(device=device, dtype=vae.dtype)
        video = video * 2.0 - 1.0
        
        # Encode and sample from latent distribution
        encoded = vae.encode(video).latent_dist.sample()
        ...
        latents = (encoded - latents_mean) * (1.0 / latents_std)
...
def save_frames(frames, target_dir: Path) -> None:
    target_dir.mkdir(parents=True, exist_ok=True)

    for idx, frame in enumerate(frames):
        if isinstance(frame, torch.Tensor):
            frame_array = frame.detach().cpu()
            if frame_array.ndim == 4 and frame_array.shape[0] == 1:
                frame_array = frame_array.squeeze(0)
            if frame_array.ndim == 3 and frame_array.shape[0] in (1, 3, 4) and frame_array.shape[0] < frame_array.shape[-1]:
                frame_array = frame_array.permute(1, 2, 0)
            frame_array = frame_array.float()
            scale = 255.0 if frame_array.max() <= 1.0 else 1.0
            frame_array = (frame_array * scale).clamp(0, 255).byte().numpy()
        else:
            frame_array = np.asarray(frame)
            if frame_array.ndim == 3 and frame_array.shape[0] in (1, 3, 4) and frame_array.shape[0] < frame_array.shape[-1]:
                frame_array = np.moveaxis(frame_array, 0, -1)
            if frame_array.dtype != np.uint8:
                scale = 255.0 if frame_array.max() <= 1.0 else 1.0
                frame_array = np.clip(frame_array * scale, 0, 255).astype(np.uint8)

        Image.fromarray(frame_array).save(target_dir / f"{idx:05d}.png")
...
def run_inference(
    ...
    # save best_idx
    import pickle
    with open(f'./best_idx_{args.name}_{args.steps}.pkl', "wb") as f:
        pickle.dump(best_idx, f)
    print(f"Saved best_idx to ./best_idx_{args.name}_{args.steps}.pkl")

    output_path = args.output_video
    output_path = output_path.parent / f"{output_path.stem}_scaling{output_path.suffix}"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    export_to_video(frames, str(output_path), fps=args.video_fps)
    

    frame_dir = output_path.parent / f"results_{args.name}_{args.steps}_scaling"
...
def main() -> None:
    ...
    if not args.disable_lora:
        ...
        print("[LoRA] LoRA adaptation is disabled - using base model only")
    
    # Load reference latent if original_frames_dir is provided
    reference_latent = None
    if args.original_frames_dir is not None:
        print(f"\nLoading reference video and encoding to latent from {args.original_frames_dir}...")
    ...
    output_path, reconstructed_frames = run_inference(
        pipe, args, device, caption_text, reference_latent=reference_latent
    )

    print(f"Saved reconstructed video to {output_path}")
    
    # Compute metrics for the returned video if requested
    if args.compute_psnr:
        ...
        print("Computing metrics for reconstructed video...")
        metrics = compute_psnr_metrics(original_frames, reconstructed_frames, device=device)
        
        print("\n" + "=" * 80)
        print("Metrics (Reconstructed Video):")
        print(f"  PSNR:         {metrics['psnr']:.4f} dB")
        print(f"  MSE:          {metrics['mse']:.6f}")
        print(f"  DISTS:        {metrics['dists']:.6f}")
        print(f"  LPIPS-VGG:    {metrics['lpips_vgg']:.6f}")
        print(f"  LPIPS-Alex:   {metrics['lpips_alex']:.6f}")
