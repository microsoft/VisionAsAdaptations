...
class Linear(nn.Linear, UniLoRALayer):
    ...
    def _get_lora_matrices(self, adapter, cast_to_fp32=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if hasattr(self, "unilora_scale_quant"):
            scale = torch.nn.functional.softplus(self.unilora_scale_quant).clamp_min(1e-6)
            norm = torch.clamp(unilora_theta_d / scale, min=-128.0, max=128.0)
            if self.training:
                quant = (torch.round(norm) - norm).detach() + norm
            else:
                quant = torch.round(norm)
            unilora_theta_d = quant * scale
