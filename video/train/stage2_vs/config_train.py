...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Weight for UniLoRA entropy penalty (0 disables entropy loss)",
    )
    parser.add_argument(
        "--lambda_rate_warmup",
        type=int,
        default=0,
        help="Steps to linearly ramp lambda_rate from 0 to full value",
    )
    parser.add_argument(
        "--num_frames",
        type=int,
    ...
    if args.lambda_rate_warmup < 0:
        raise ValueError("lambda_rate_warmup must be non-negative")
