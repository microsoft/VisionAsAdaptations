...
Clean copy for Compression_clean stage-1 training.
"""
...
def main():
    ...
    if rank == 0:
        print("=== Training UniLoRA on Wan2.1 ===")
    ...
    ############## Base model parameter count and size ##########
    if rank == 0:
        base_num_params = sum(p.numel() for p in pipe.transformer.parameters())
        bytes_per_param = 2 if dtype == torch.bfloat16 else 4
        base_size_bytes = base_num_params * bytes_per_param
        base_size_mb = base_size_bytes / (1024 ** 2)
        base_size_gb = base_size_bytes / (1024 ** 3)
        size_str = f"{base_size_gb:.2f} GB" if base_size_gb >= 1 else f"{base_size_mb:.2f} MB"
        print('###################################################')
        print(f"[Base model] Parameters: {base_num_params:,} ({base_num_params/1e6:.2f}M), Size: {size_str}")
        print('###################################################')
    ##########
    ...
    for step in range(start_step + 1, args.train_steps + 1):
        ...
        if rank == 0:
            if step % 1 == 0 or step == 1:
                print(
                    (
                        f"[{step}/{args.train_steps}] loss={avg_loss:.4f} model_loss={avg_model_loss:.4f} "
    ...
    if rank == 0 and args.generate_samples:
